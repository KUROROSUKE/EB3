    ...
    p2_hand.forEach((elem, index) => {
        ...
        image.addEventListener("click", function() {
            ...
            if (turn == "p2" && time == "game") {
                ...
                newImg.src = URL.createObjectURL(newBlob);
                newImg.alt = newElem;
...
async function p1_exchange(targetElem) {
    ...
    const newBlob2 = imageCache[elementToNumber[newElem]];
    const newImg2 = new Image();
    newImg2.src = URL.createObjectURL(newBlob2);
    newImg2.alt = newElem
    newImg2.style.padding = "5px"
    newImg2.style.border = "1px solid #000"
    // Remove and reapply the 'selected' class to reset the state
    newImg2.classList.remove("selected")
    newImg2.classList.add("selected")
